...
then
  ...
  VIGRID_NICS=`ls -l /sys/class/net/ 2>/dev/null | egrep -v "(virtual|^total )"| sed 's/->.*$//' | awk '{print $NF;}'`
  # VIGRID_NICS=`ip link | egrep "^[0-9]*:" | egrep -v "(virbr[0-9].*|gns3tap[0-9].*|docker[0-9]*|lo): <"| awk '{print $2;}' | sed 's/://g'`
  ...
  then
    ...
    do
      ...
      do
        NIC_IP_FULL=`ip addr show $i|grep "inet" |head -1| awk '{print $2;}'`
...
then
  cd /home/gns3/vigrid && git config pull.rebase false && git pull || echo Vigrid update failed
else
  cd /home/gns3 && git clone https://github.com/llevier/vigrid.git --branch v1.0 || echo Vigrid update failed
fi
...
WSC=`curl -s -o - https://github.com/vi/websocat/releases|grep "\.deb\"" 2>/dev/null|grep "amd64"|head -1 | sed 's/^.*href=\"//i' | sed 's/\".*$//'`
WSC_FILE=`basename $WSC`
rm $WSC_FILE >/dev/null 2>/dev/null
cd /tmp && wget --progress=bar:force:noscroll -c "https://github.com/$WSC" && dpkg -i $WSC_FILE || Error 'Install failed,'
rm $WSC_FILE
cd $SCRIPT_CWD
...
if [ $VIGRID_TYPE -ge 1 -a $VIGRID_TYPE -le 3 ]
then
  Display "Installing PHP FPM..." && apt install -y php-fpm php-curl php-mail php-net-smtp php-react-child-process || Error 'Install failed,'
...
for i in /home/gns3/vigrid/etc/init.d/bwm*
do
  SERVICE=`basename $i`
  cp $i /etc/init.d/
...
LIST="$LIST ssh bwmNG-disk-max bwmNG-disk-rate bwmNG-net-max bwmNG-net-rate docker gns3"
if [ $VIGRID_TYPE -ge 1 -a $VIGRID_TYPE -le 3 ]
then
